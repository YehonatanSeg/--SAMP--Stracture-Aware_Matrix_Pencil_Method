...
params.CLUST_DIST_RANGE = 1.5*pi/(params.dt*N):pi/(params.dt*N):15*pi/(params.dt*N);
SNR = params.SNR_RANGE;
params.FREQ_DIST = 1.5*pi/(params.dt*N);
params.true_amps   = [1, 1, 1, 1];
params.true_phases = [0, 0, 0, 0];
params.true_damps  = damps;

Left_modes      = cell(length(params.CLUST_DIST_RANGE), params.Nrep);
Poles           = cell(length(params.CLUST_DIST_RANGE), params.Nrep);
Amplitudes   = cell(length(params.CLUST_DIST_RANGE), params.Nrep);

ind = 1;
for clust_dist = params.CLUST_DIST_RANGE  
    w2 = w1  + params.FREQ_DIST + clust_dist;
    params.true_freqs  = [w2 + params.FREQ_DIST, w2, w1 + params.FREQ_DIST, w1 ] ;
    params.true_poles = exp(-params.true_damps*params.dt + 1i*params.true_freqs*params.dt);

    for rep = 1:params.Nrep
        disp(['Dampings = [', num2str(damps), ']', ' clusters distance = ', num2str(clust_dist), ' interval length = ', num2str(params.interval_length), ' SNR = ', num2str(SNR), ' rep = ', num2str(rep)])
